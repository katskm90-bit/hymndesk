...
const CACHE_NAME = 'hymndesk-v3';
const HYMNS_CACHE = 'hymndesk-hymns-v3';
...
self.addEventListener('fetch', function(event) {
  ...
  // Only intercept same-origin /hymns.json — never intercept raw.githubusercontent.com
      ...
      fetch(new Request(event.request.url, {
        ...
        .then(function(response) {
          if (response && response.status === 200) {
            ...
            caches.open(HYMNS_CACHE).then(function(c) { c.put('/hymns.json', clone); });
          }
          return response;
        })
        .catch(function() {
...
self.addEventListener('periodicsync', function(event) {
  if (event.tag === 'hymndesk-hymn-refresh') {
    event.waitUntil(
      fetch('/hymns.json?t=' + Date.now(), { cache: 'no-store' })
        .then(function(response) {
          if (response && response.status === 200) {
            var clone = response.clone();
            return caches.open(HYMNS_CACHE).then(function(c) {
              return c.put('/hymns.json', clone);
            });
          }
        })
        .catch(function() {})
